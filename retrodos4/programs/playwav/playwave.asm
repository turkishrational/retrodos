...
; ----------------------------------------------------------------------------
; Last Update: 04/11/2023 (Previous: 17/02/2017)
; ----------------------------------------------------------------------------
...
[ORG 100h] 
	...
	%include 'ac97.inc' ; 17/02/2017 ; 03/11/2023
...
ac97_int_handler:
	...
	; 28/05/2017 (TDOS 386 v2, 'audio.s')
	...
	test	eax, eax
	jz	short _ac97_ih5  ; exit
	
 	;mov	dx, GLOB_STS_REG
        ;add	dx, [NABMBAR]
	out	dx, eax
	
	jmp	short _ac97_ih5	 ; exit
...
_ac97_ih0:
	...
	inc	al
	mov	[audio_flag], al 
	;; [audio_flag] : 1 = Buffer 1, 2 = Buffer 2
	;
	pop	eax
_ac97_ih1:
	; 04/11/2023
...
ich_init:
	; 03/11/2023 (Ref: MPXPLAY source code, 'SC_ICH.C' file)

	ICH_GLOB_STAT_PCR equ    100h  ; primary codec is ready
	ICH_GLOB_STAT_RCS equ   8000h  ; completion status
	ICH_PCM_246_MASK  equ 300000h  ; 6 channels
	ICH_GLOB_CNT_ACLINKOFF equ 8h  ; turn off ac97 link
 		
	mov	dx, [NABMBAR]
	add	dx, GLOB_STS_REG
	in	eax, dx
	and	eax, ICH_GLOB_STAT_RCS
	out	dx, eax

	mov	dx, [NABMBAR]
	add	dx, GLOB_CNT_REG
	in	eax, dx
	and	eax, ~(ICH_GLOB_CNT_ACLINKOFF | ICH_PCM_246_MASK)
...
flags:		resb 1
; 04/11/2023
audio_flag:	resb 1	; (buffer 1 or buffer 2)

;irq_status:	resb 1	; AC97 IRQ status
;inside:	resb 1
;tLoop:		resb 1	
