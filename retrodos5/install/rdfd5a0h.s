; ****************************************************************************
; RDFD5A0H.ASM (RDFD5A0H.COM) - Retro DOS v5 Floppy Disk Boot Sector Utility
; (R5FDBOOT.ASM)					 (for MSDOS/WINDOWS)
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Modified from 'rdfd1440.s'(RDFD1440.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; assembling: nasm rdfd5a0h.s -l rdfd5a0h.txt -o RDFD5A0H.COM -Z error.txt
...
T_06:
	...
	jne	short T_22
	; 25/10/2023
	; check 1.44 MB FD (required) FAT12 BPB parameters
	cmp	byte [bootsector+bsMedia], 0F0h
	jne	short T_22	
	cmp	byte [bootsector+bsSecPerClust], 1
	jne	short T_22
	cmp	word [bootsector+bsSectors], 2880
	jne	short T_22
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'Retro DOS v5 - 1.44 MB Floppy Disk Boot Sector Update Utility'
	db	0Dh, 0Ah
	;db	"v1.0.231025  (c) Erdogan TAN 2023"
	db	"RDFDBOOT v2.0.240420  (c) Erdogan TAN 2023-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rdfd5a0h a: (or b:)'
	db	0
...
RD_invalid_bootsector:
	...
	db 0Dh,0Ah
	db '(! not a valid 1.44 MB FAT12 floppy disk !)'
	db 0
msg_NO:
	db	' NO ..', 0Dh, 0Ah, 0
msg_YES:
...
;incbin	"FDBS1440.BIN"	; Kernel file: 'MSDOS.SYS'
incbin	"FDBS5A0H.BIN"	; Kernel file: 'PCDOS.SYS' ; 20/04/2024
