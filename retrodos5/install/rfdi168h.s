; ****************************************************************************
; RFDI168H.ASM (RFDI168H.COM) - Retro DOS v5.0 Floppy Disk Image Formatting
...
; ----------------------------------------------------------------------------
; Modified from 'rfdi360.s'(RFDI360.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; nasm rfdi168h.s -l rfdi168h.txt -o RFDI168H.COM

; rfdi360.s - Retro DOS v4.0-v4.2 - 25/10/2023
...
R_14:
	...
	cmp	word [SI], 5
	jne	short R_19
	cmp	ax, 0A000h ; 360KB floppy disk image (5A000h bytes)
	jne	short R_19
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	mov	ax, 5  ; Root Directory Address
	mov	bx, FDFORMAT_FATBUFFER_S9
R_22:
	call	write_fd_sector
	jc	R_29
	inc	ax
 	cmp	ax, 11 ; 5+7 = 12 (7 sectors, 112 entries)
	jna	short R_22
...
RETRODOS_FAT12_FDBS:
	...
	incbin	'FDBS168H.BIN'	; 20/04/2024
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'RETRO DOS v5 360KB FAT12 Floppy Disk Image Format Utility'
	db	0Dh, 0Ah
	;db	"v3.0.231025  (c) Erdogan TAN 2018-2023"
	db	"RDFDIMAGE v4.0.240420  (c) Erdogan TAN 2018-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rfdi168h <image file name> '
	db	0
...
msg_inv_image_file:
	...
	db	"(File size must be 368640 bytes) !"
	db	0Dh, 0Ah, 0  
