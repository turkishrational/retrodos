; ****************************************************************************
; RDFDB40H.ASM (RDFDB40H.COM) - Retro DOS v5 Floppy Disk Boot Sector Utility
;							 (for MSDOS/WINDOWS)
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Modified from 'rdfd2880.s'(RDFD2880.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; assembling: nasm rdfdb40h.s -l rdfdb40h.txt -o RDFDB40H.COM -Z error.txt
...
T_06:
	...
	jne	short T_22
	; 25/10/2023
	; check 2.88 MB FD (required) FAT12 BPB parameters
	cmp	byte [bootsector+bsMedia], 0F0h
	jne	short T_22	
	cmp	byte [bootsector+bsSecPerClust], 2
	jne	short T_22
	cmp	word [bootsector+bsSectors], 5760
	jne	short T_22
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'Retro DOS v5 - 2.88 MB Floppy Disk Boot Sector Update Utility'
	db	0Dh, 0Ah
	;db	"v1.0.231025  (c) Erdogan TAN 2023"
	db	"RDFDBOOT v2.0.240420  (c) Erdogan TAN 2023-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rdfdb40h a: (or b:)'
	db	0
...
RD_invalid_bootsector:
	...
	db 0Dh,0Ah
	db '(! not a valid 2.88 MB FAT12 floppy disk !)'
	db 0
msg_NO:
	db	' NO ..', 0Dh, 0Ah, 0
msg_YES:
...
;incbin	"FDBS2880.BIN"	; Kernel file: 'MSDOS.SYS'
incbin	"FDBSB40H.BIN"	; Kernel file: 'PCDOS.SYS' ; 20/04/2024
