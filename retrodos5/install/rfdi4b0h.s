; ****************************************************************************
; RFDI4B0H.ASM (RFDI4B0H.COM) - Retro DOS v5.0 Floppy Disk Image Formatting
;						  Utility for MSDOS/WINDOWS
; ----------------------------------------------------------------------------
; Only for 1.2MB (5 1/4") Floppy Disks
; ****************************************************************************
...
; ----------------------------------------------------------------------------
; Modified from 'rfdi1200.s'(RFDI1200.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; nasm rfdi4b0h.s -l rfdi4b0h.txt -o RFDI4B0H.COM

; rfdi1200.s - Retro DOS v4.0-v4.2 - 25/10/2023
...
R_14:
	...
	cmp	word [SI], 12h
	jne	short R_19
	cmp	ax, 0C000h ; 1.2 MB floppy disk image (12C000h bytes)
	jne	short R_19
...
;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	mov	ax, 15  ; Root Directory Address
	mov	bx, FDFORMAT_FATBUFFER_S9
R_22:
	call	write_fd_sector
	jc	R_29
	inc	ax
 	cmp	ax, 28 ; 15+14 = 29 (14 sectors, 224 entries)
	jna	short R_22
...
RETRODOS_FAT12_FDBS:
	;incbin 'FDBS1200.BIN'	; 25/10/2023
	incbin	'FDBS4B0H.BIN'	; 20/04/2024
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'RETRO DOS v5 1.2MB FAT12 Floppy Disk Image Format Utility'
	db	0Dh, 0Ah
	;db	"v3.0.231025  (c) Erdogan TAN 2018-2023"
	db	"RDFDIMAGE v4.0.240420  (c) Erdogan TAN 2018-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rfdi4b0h <image file name> '
	db	0
...
msg_inv_image_file:
	...
	db	"(File size must be 1228800 bytes) !"
	db	0Dh, 0Ah, 0
