; ****************************************************************************
; RD5HDB16.ASM (RD5HDB16.COM) - Retro DOS v5 Hard Disk FAT16 Boot Sect Utility
; 					 	          (for MSDOS/WINDOWS)
; ****************************************************************************
...
; ----------------------------------------------------------------------------
; Modified from 'r4hdboot.s'(R4HDBOOT.COM) source code by Erdogan Tan
; (25/10/2023) - Retro DOS v2-v4 hard disk boot sector modification utility -
; ****************************************************************************
; nasm rd5hdb16.s -l rd5hdb16.txt -o RD5HDB16.COM

; 21/10/2023
;Note: FAT16 BIG (>32MB) partitions are already recognized by Retro DOS v3.2
;      But MSDOS 3.3 can use only 16 bit disk addresses (FAT16 fs ID=04h)
;      As result, 'R4HDBOOT.COM' is for Retro DOS v4 (MSDOS 5.0-6.22)
;      and Retro DOS v3.2 (MSDOS 3.3+).
;
;      Retro DOS v3 hd boot sector is compatible with FAT16 BIG fs (ID=06h)
;      Retro DOS v2 hd boot sector is used for FAT12 fs (ID=01h) only.
...
R_07:
	...
	cmp	al, 4
	; 20/10/2018
	;cmp	byte [bx+ptFileSystemID], 4  ; MSDOS (FAT16) Partition ID
	je	short R_23
	; 21/10/2023
	cmp	al, 6
	;cmp	byte [bx+ptFileSystemID], 6  ; MSDOS (FAT16 BIG) Partition ID
	je	short R_23
...
R_18:
	...
	; 21/10/2023
	; (32 bit sector addresses are valid for Retro DOS v4
	;  -MSDOS 5.0 to 6.22-)
R_23:
	; 20/10/2018
	mov	ax, RETRODOS_FAT16_HD_BS
	mov	[RD3BOOTSECTOR], ax
R_08:
	mov	ax, [bx+ptStartSector]
	mov	dx, [bx+ptStartSector+2]

; 21/10/2023 - Retro DOS v4.0-4.2 Boot Sector Update(r) Utility
; (16 bit sector address limit was an issue before MSDOS 5.0)
%if 0

	and	dx, dx
	;jnz	R_19    ; Not a valid MSDOS (FAT12/FAT16) partition
	; 21/10/2023
	jnz	short R_19

	cmp	word [bx+ptSectors+2], 0
	;ja	R_19    ; Not a valid MSDOS (FAT12/FAT16) partition
	; 21/10/2023
	ja	short R_19

	mov	dx, ax
	add	dx, [bx+ptSectors]
	;jc	R_19    ; Not a valid MSDOS (FAT12/FAT16) partition
	; 21/10/2023
	jc	short R_19
%endif
	...
	; 21/10/2023
	mov	cx, [bx+ptSectors]
	mov	bx, [bx+ptSectors+2]
	add	cx, ax
	adc	bx, dx
	jc	short R_19 ; invalid partition table entry !
...
R_25:
	cmp	word [bx+510], 0AA55h 
	;jne	R_19  ; Not a valid MSDOS (FAT12/FAT16) partition
	jne	short R_19 ; 21/10/2023
...
R_13:
	...
	; 20/10/2018
	mov	si, [RD3BOOTSECTOR]
	;mov	[RETRODOS_FAT12_HD_BS+bsRootDirStart], ax
	; 15/09/2022
	mov	[si+bsRootDirStart], ax
	...
	mov	[si+bsRootDirSects], cx

	; 22/10/2023
	inc	dx ; dx = 16
	;mov	word [si+bsDirEntsPerSec], 16
	mov	[si+bsDirEntsPerSec], dx

	add	ax, cx
	;mov	[RETRODOS_FAT12_HD_BS+bsDataStart], ax
	mov	[si+bsDataStart], ax
	...
	;mov	di, [RD3BOOTSECTOR]
	; 22/10/2023
	mov	di, si ; [RD3BOOTSECTOR]
	push	di
	add	di, bsVolumeLabel
...
convert_to_chs:
	...
	; 21/10/2023 - Retro DOS v4 (32 bit disk sector addresses)
	; bx:cx = end of the partition (the last sector addr + 1)
	; dx:ax = start sector address of the partition
	...
	jnz	short ctchs4
	; dx = 0
	; 16 bit sector number (absolutely within CHS limit)
	pop	ax ; *

	div 	word [Sectors] ; <= 63
	inc	dl	; 0 to 62 -> 1 to 63
	mov	cl, dl	; sector
	xor	dl, dl	; (dh = 0)
	jmp	short ctchs_5 ; skip 32 bit divisions
...
ctchs0:
	...
	je	short ctchs1
ctchs3:
	; not possible to use CHS values !!
	; cf = 1
	pop	dx ; **
	pop	ax ; *
	retn
ctchs1:
	...
	; 21/10/2023
	cmp	ax, cx
	jb	short ctchs3 ; Not a valid MSDOS (FAT12/FAT16) partition
ctchs2:
	; 21/10/2023
	pop	dx ; **
ctchs4:
	;mov	ax, bx
	; 21/10/2023 - 32 bit divide
	; sample: (5:9)/4, 5/4 -> remainder = 1, (1:9)/4 -> remainder = 3
	;	  quotient = (1:4), remainder = 3 (sector number-1)
	;push	ax ; *
	; [sp] = ax (lw)
	mov	ax, dx	; (hw)	;; 5
	xor	dx, dx  ; 0
	; AX = LBA sector address (DX = 0)
	div 	word [Sectors]	;; 5/4 
	mov	bx, ax		;; ax = 1
				;; dx = 1 (5%4)
	pop	ax ; *		;; ax = 9, dx = 1
	div 	word [Sectors]	;; 19/4
				;; ax = 4, dx = 3
				;; bx:ax = 14  
	inc	dl		;; 3 -> 4
	mov	cl, dl ; sector	; <= 63
	...
	xor	dl, dl	; dx = 0
	; 21/10/2023
	; sample: (1:4)/2 (dividend = 14, high nibble = 1, low nibble = 4)
	push	ax ; +		;; 4 ; ln
	mov	ax, bx ; hn	;; 1
	div	word [Heads]	;; 1/2 (heads=2)
				;; ax = 0, dx = 1
	pop	ax ; +	; ln	;; 4
ctchs_5:	; 21/10/2023
	;;;
	div	word [Heads] ; [BPB_NumHeads] ;; 14/2, ax = 7, dx = 0
...
RD3BOOTSECTOR:
	dw	RETRODOS_FAT12_HD_BS

RETRODOS_FAT16_HD_BS:
	;;incbin 'RD3HDBS.BIN'
	; 21/10/2023 - 22/10/2023
	;incbin	'RD4HDBS.BIN'	; boot sector : 24/10/2023
	incbin	'RD5HDBS2.BIN'	; FAT16 hd boot sector ; 20/04/2024
...
RetroDOS_Welcome:
	...
	db	"RDHDBOOT v1.3.240420  (c) Erdogan TAN 2018-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rd5hdb16 c: (or d:)'
	db	0
...
msg_msdos_p_not_found:
	db	0Dh, 0Ah
	db	"MSDOS (FAT12/FAT16) partition not found !"
	db	0Dh, 0Ah, 0 

	; 21/10/2023
msg_not_valid_vbr:
	db	0Dh, 0Ah
	;db	"Not a valid MSDOS (FAT12/FAT16) partition ! (for Retro DOS v4)"
	; 20/04/2024
	db	"Not a valid MSDOS (FAT12/FAT16) partition ! (for Retro DOS v5)"
	db	0Dh, 0Ah, 0 
...
msg_writing_boot_sector:
	;db	"Updating boot sector to Retro DOS v4 format ...", 0
	; 20/04/2024
