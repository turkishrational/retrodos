; ****************************************************************************
; RDFD2D0H.ASM (RDFD2D0H.COM) - Retro DOS v5 Floppy Disk Boot Sector Utility
; 							 (for MSDOS/WINDOWS)
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Modified from 'rdfd720.s'(RDFD720.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; assembling: nasm rdfd2d0h.s -l rdfd2d0h.txt -o RDFD2D0H.COM -Z error.txt
...
T_06:
	...
	jne	short T_22
	; 25/10/2023
	; check 720 KB FD (required) FAT12 BPB parameters
	cmp	byte [bootsector+bsMedia], 0F9h
	jne	short T_22	
	cmp	byte [bootsector+bsSecPerClust], 2
	jne	short T_22
	cmp	word [bootsector+bsSectors], 1440
	jne	short T_22
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'Retro DOS v5 - 720 KB Floppy Disk Boot Sector Update Utility'
	db	0Dh, 0Ah
	;db	"v1.0.231025  (c) Erdogan TAN 2023"
	db	"RDFDBOOT v2.0.240420  (c) Erdogan TAN 2023-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rdfd2d0h a: (or b:)'
	db	0
...
RD_invalid_bootsector:
	...
	db 0Dh,0Ah
	db '(! not a valid 720 KB FAT12 floppy disk !)'
	db 0
msg_NO:
	db	' NO ..', 0Dh, 0Ah, 0
msg_YES:
...
;incbin	"FDBS720.BIN"	; Kernel file: 'MSDOS.SYS'
incbin	"FDBS2D0H.BIN"	; Kernel file: 'PCDOS.SYS' ; 20/04/2024
