; ****************************************************************************
; R5FDBOOT.ASM (R5FDBOOT.COM) - Retro DOS v5 Floppy Disk Boot Sector Utility
;						 	 (for MSDOS/WINDOWS)
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Modified from 'rdfdboot.s'(RDFDBOOT.COM) source code by Erdogan Tan
; (25/10/2023) - RETRODOS v4 floopy disk boot sector modification utility -
; ****************************************************************************
; assembling: nasm r5fdboot.s -l r5fdboot.lst -o R5FDBOOT.COM -Z error.txt
...
T_06:
	...
	jne	short T_22
	; 25/10/2023
	; check parameters depending on diskette size (total sectors)
	mov	si, [drive_type]
	dec	si  ; 1 -> 0
	shl	si, 1
	add	si, capacities
...
chk_cap_ok:
	...
	sub	bx, cx ; cx > 0 & <= 5
	shl	bx, 1
	mov	ax, [bx+drive_compat]
	cmp	al, [drive_type]
	je	short chk_parms
	cmp	ah, [drive_type]
	jne	short T_22 ; not compatible
chk_parms:
	lea	si, [bx+parameters]
	lodsw
	cmp	al, [bootsector+bsMedia]
	jne	short T_22 ; invalid
	cmp	ah, [bootsector+bsSecPerClust]
	jne	short T_22 ; invalid
...
R_12:
	...
	; check drive type 5 (2.88MB)
	cmp	byte [drive_type], 5
	jne	short copy_bpb

	add	bx, 512 ; 2.88 MB RDBS

copy_bpb:
	; move/copy drive number to RDBS
	mov	al, [DriveNumber]
	mov	[bx+bsDriveNumber], al

	; copy the disk's BPB to the new Retrodos v4 boot sector
	lea	si, [bootsector+bsOemName] 
	lea	di, [bx+bsOemName]
	mov	cx, bsFileSysType - bsOemName
	rep	movsb
R_13:
	; Calculate Retro DOS v4 extended BS parameters
	mov	ax, [bx+bsFATsecs]
	mov	cl, [bx+bsFATs]
	;mul	cx
	dec	cl
	...
	mov	[bx+bsRootDirSects], cx

	inc	dx ; dx = 16
	mov	[bx+bsDirEntsPerSec], dx

	add	ax, cx
	mov	[bx+bsDataStart], ax
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'Retro DOS v5 Floppy Disk Boot Sector Update Utility'
	db	0Dh, 0Ah
	;db	"v1.0.231025  (c) Erdogan TAN 2023"
	db	"RDFDBOOT v3.0.240420  (c) Erdogan TAN 2023-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: r5fdboot a: (or b:)'
	db	0
...
RD_invalid_bootsector:
	...
	db	0Dh,0Ah
	db	'(! not a valid FAT12 -DOS- floppy disk !)'
	db	0
msg_NO:
	db	' NO ..', 0Dh, 0Ah, 0
msg_YES:
...
;incbin	"FDBS1440.BIN"	; Kernel file: 'MSDOS.SYS' ; common bs except 2.88 MB 
;incbin	"FDBS2880.BIN"	; Kernel file: 'MSDOS.SYS' ; only for 2.88 MB
; 20/04/2024
incbin	"FDBS5A0H.BIN"	; Kernel file: 'PCDOS.SYS' ; common bs except 2.88 MB 
incbin	"FDBSB40H.BIN"	; Kernel file: 'PCDOS.SYS' ; only for 2.88 MB

RetryCount:
	db	0

; structures for common (multiple format) parameters
; 25/10/2023 
