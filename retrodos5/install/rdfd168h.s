; ****************************************************************************
; RDFD168H.ASM (RDFD168H.COM) - Retro DOS v5 Floppy Disk Boot Sector Utility
; 						 	 (for MSDOS/WINDOWS)
; ----------------------------------------------------------------------------
...
; ----------------------------------------------------------------------------
; Modified from 'rdfd360.s'(RDFD360.COM) src code by Erdogan Tan, 25/10/2023
; ****************************************************************************
; assembling: nasm rdfd168h.s -l rdfd168h.txt -o RDFD168H.COM -Z error.txt
...
T_06:
	...
	jne	short T_22
	; 25/10/2023
	; check 360 KB FD (required) FAT12 BPB parameters
	cmp	byte [bootsector+bsMedia], 0FDh
	jne	short T_22	
	cmp	byte [bootsector+bsSecPerClust], 2
	jne	short T_22
	cmp	word [bootsector+bsSectors], 720
	jne	short T_22
...
RetroDOS_Welcome:
	db	0Dh, 0Ah
	db	'Retro DOS v5 - 360 KB Floppy Disk Boot Sector Update Utility'
	db	0Dh, 0Ah
	;db	"v1.0.231025  (c) Erdogan TAN 2023"
	db	"RDFDBOOT v2.0.240420  (c) Erdogan TAN 2023-2024"
	db	0Dh,0Ah
	db	0Dh,0Ah
	db	'Usage: rdfd168h a: (or b:)'
	db	0
...
RD_invalid_bootsector:
	...
	db 0Dh,0Ah
	db '(! not a valid 360 KB FAT12 floppy disk !)'
	db 0
msg_NO:
	db	' NO ..', 0Dh, 0Ah, 0
msg_YES:
...
;incbin	"FDBS360.BIN"	; Kernel file: 'MSDOS.SYS'
incbin	"FDBS168H.BIN"	; Kernel file: 'PCDOS.SYS' ; 20/04/2024
