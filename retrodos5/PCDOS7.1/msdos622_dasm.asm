...
GOT_VALS:				; ...
		...
		cmp	ah, 0FFh
		jz	short NO_SET_ACT
		mov	byte ptr ds:EXTERR_ACT_CLASS, ah

NO_SET_ACT:				; ...
		cmp	al, 0FFh
		jz	short NO_SET_CLS
		mov	byte ptr ds:EXTERR_ACT_CLASS+1,	al
...
Win386_vchk:				; ...
		mov	word ptr ds:Win386_Info_6, 0
		mov	word ptr ds:Win386_Info_6+2, 0
		cmp	di, 30Ah
		jb	short Win386_vxd
		jmp	noVxD31
; ---------------------------------------------------------------------------
...
setglpg:				; ...
		...
		mov	ax, 65
		mov	ds:EXTERR, ax
		mov	byte ptr ds:EXTERR_ACT_CLASS, 6
		mov	byte ptr ds:EXTERR_ACT_CLASS+1,	5
...
$GET_IN_VARS:				; ...
		call	Get_User_Stack
		mov	word ptr [si+2], offset	DPBHEAD	; SYSINITVARS
		mov	word ptr [si+10h], ss
		retn
; ---------------------------------------------------------------------------
...
addr_ok:				; ...
		...
		mov	word ptr ds:32h, 20
		mov	word ptr ds:38h, 0FFFFh
		mov	word ptr ds:3Ah, 0FFFFh
		mov	word ptr es:40h, 1606h ; 6.22
		retn
...
ECritDisk_iret2	proc far		; ...
		iret
ECritDisk_iret2	endp
...
LCritDisk_iret2	proc far		; ...
		iret
LCritDisk_iret2	endp
...
ECritDevice_iret proc far		; ...
		iret
ECritDevice_iret endp
...
ECritDevice_1:				; ...
		push	cs
		call	near ptr ECritDevice_iret
		push	ax
		mov	ax, 8002h
		int	2Ah		; Microsoft Networks - BEGIN DOS CRITICAL SECTION
					; AL = critical	section	number (00h-0Fh)
...
ECritDevice_iret2 proc far		; ...
		iret
ECritDevice_iret2 endp
...
ECritDevice_3:				; ...
		push	cs
		call	near ptr ECritDevice_iret2
		retn
; END OF FUNCTION CHUNK	FOR ECritDevice
...
LCritDevice_iret proc far		; ...
		iret
LCritDevice_iret endp
...
LCritDevice_1:				; ...
		push	cs
		call	near ptr LCritDevice_iret
		push	ax
		mov	ax, 8102h
		int	2Ah		; Microsoft Networks - END DOS CRITICAL	SECTION
					; AL = critical	section	number (00h-0Fh)
...
LCritDevice_iret2 proc far		; ...
		iret
LCritDevice_iret2 endp
...
LCritDevice_3:				; ...
		push	cs
		call	near ptr LCritDevice_iret2
		retn
; END OF FUNCTION CHUNK	FOR LCritDevice
...
noscan:					; ...
		retn
$STD_CON_INPUT_NO_ECHO endp ; sp-analysis failed
...
ENDLIN:					; ...
		...
		mov	[di-1],	dh
		inc	dh
$STD_CON_STRING_INPUT endp ; sp-analysis failed
...
BACKUP		proc near		; ...
		dec	dh
		dec	di
BACKUP		endp ; sp-analysis failed
...
RAWOUT2		proc near		; ...
		call	GET_IO_SFT
		jb	short RAWRET2
RAWOUT2		endp ; sp-analysis failed
...
SetIOCtlBlock:				; ...
		...
		mov	ds:IOMED, ch
		mov	byte ptr ds:IOXAD, cl
		mov	word ptr ds:IOXAD+1, si
		mov	word ptr ds:IOXAD+3, di
		mov	ds:IOSCNT+1, dx
		mov	ds:IOSSEC+1, si
...
ioctl_control_call:			; ...
		...
		mov	ds:IOMED, al
		mov	ds:IOSCNT, cx
		mov	word ptr ds:IOXAD, dx
		mov	word ptr ds:IOXAD+2, si
		push	es
		pop	ds
		mov	si, di
		push	ss
...
REN_DEL_Check	proc near		; ...
		...
		mov	word ptr ds:THISSFT, (offset RENAMEDMA+145h) ; AUXSTACK-SF_ENTRY.size
					; RENAMEDMA+(384-59)
		xor	ah, ah
		call	DOOPEN
		les	di, ds:THISSFT
...
_BAD_ACC:				; ...
		mov	ds:EXTERR_LOCUS, 1
		mov	byte ptr ds:EXTERR_ACT_CLASS+1,	7
		mov	byte ptr ds:EXTERR_ACT_CLASS, 4
		mov	ax, 5
		stc
		retn
; ---------------------------------------------------------------------------
...
NXTFAT:					; ...
		...
		pop	dx
		mov	cx, di
FATSECRD	endp ; sp-analysis failed
...
RDERR:					; ...
		mov	ah, 0Eh
		jmp	WRTERR22
DISKREAD	endp ; sp-analysis failed
...
SET_ACC_ERR_DS	proc near		; ...
		push	ss
		pop	ds
SET_ACC_ERR_DS	endp ; sp-analysis failed
...
SETCLUS:				; ...
		...
		mov	ax, ds:LASTPOS
		mov	es:[di+19h], ax
SETSFT		endp ; sp-analysis failed
...
WRTERR:					; ...
		mov	ah, 0Fh
DISKWRITE	endp ; sp-analysis failed
...
CheckThisDevice	endp ; sp-analysis failed
...
SETXADDR	proc near		; ...
		...
		pop	ds
		mov	word ptr ds:THISSFT, si
		mov	cx, word ptr ds:IOXAD+2
		mov	word ptr ds:DMAADD+2, cx
		mov	cx, word ptr ds:IOXAD
...
chklow:
		...
		mov	word ptr ss:CALLBPB, ax	; [ss:CALLSCNT]
...
VIRREAD		proc near		; ...
		...
		mov	word ptr ss:CALLBR, ax
		mov	ss:SC_FLAG, 0
		mov	word ptr ss:CALLBPB, 1 ; [ss:CALLSCNT]
...
noSC:					; ...
		stc
		retn
SC2BUF		endp ; sp-analysis failed
...
; ---------------------------------------------------------------------------
LowInt23Addr	dd offset LowInt23	; ...
LowInt24Addr	dd offset LowInt24	; ...
LowInt28Addr	dd offset LowInt28	; ...
...
_OUTMES		proc near		; ...
		...
		mov	ds:DSKSTCNT, bx
		mov	bx, offset DSKSTCALL
		mov	ds:DEVIOBUF_PTR, si ; [DSKCHRET+1]
		mov	ds:DOSSEG_INIT,	cs ; [DSKCHRET+3]
		lds	si, ds:BCON
		call	DEVIOCALL2
...
FATAL1:					; ...
		...
		les	si, es:[bp+13h]
		mov	bp, es
HARDERR		endp ; sp-analysis failed
...
CHECK_BYT:				; ...
		or	ah, byte ptr ds:CALLBR ; [CALLRBYT]
		jns	short CHECK_ZR
		jmp	short NEWDSK
; ---------------------------------------------------------------------------
...
nxbuffer:				; ...
		cmp	[di+4],	al
		jnz	short lfnxt2
		test	byte ptr [di+5], 40h ; buf_dirty
...
GOTGETBUF:				; ...
		add	di, 20		; BUFINSIZ
		mov	word ptr ss:CALLBR+2, ds ; [SS:CALLXAD+2]
		push	ss
		pop	ds
		mov	word ptr ds:CALLBR, di ; [CALLXAD]
		mov	al, 16h
		mov	ah, es:[bp+1]	; ES:BP+DPB.UNIT]
		mov	word ptr ds:DEVCALL, ax	; [DEVCALL_REQLEN]
...
free_the_buf:				; ...
		mov	word ptr [di+4], 0FFh ;	[DI+BUFFINFO.buf_ID]
...
exec_read_reloc:			; ...
		push	dx
		mov	dx, offset OPENBUF ; Exec_Internal_Buffer
...
Exec_Set_PDB:				; ...
		...
		pop	bp
		call	Exec_Alloc
		test	byte ptr [bp-5], 2 ; exec_func_overlay
...
arena_next	proc near		; ...
		...
		inc	ax
arena_next	endp ; sp-analysis failed
...
DOSINIT:				; ...
		...
		assume es:nothing
		mov	es, word ptr es:3 ; DosDataSg equ 3
		assume es:nothing
		xor	di, di
...
di_set_patch:				; ...
		...
		mov	ds:SIS_Instance_Data_Ptr_2, ds
		push	si
		mov	cx, 7
...
SysInitTable	dd offset DPBHEAD	; ...
		dd offset COUNTRY_CDPG
FastOpenTable	dw 2			; ...
