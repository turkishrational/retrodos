...
3) Retro DOS v3.2 - 32 bit disk sector addresses (>32 MB disk/partition access)
		    ...
		    - September 2022- 	
4) Retro DOS v3.3 - Windows 3.1 compatible version (Previous one is not proper
		    for running windows 3.1). 16 bit disk sector addresses.
		    Disassembled (and edited) kernel file (msdos33.asm) is
		    almost same with PC-DOS 3.30 IBMDOS.COM (disassembled)
		    kernel source (procedure/subroutine addresses and instructions
		    are need to be same for running Windows 3.1). Original 
		    'COMMAND.COM' is needed to run at dos prompt in Windows
		    properly.	
		    -Summary: Modified Retro DOS v3.0 for running Windows 3.1-		 		
Kernel files:
