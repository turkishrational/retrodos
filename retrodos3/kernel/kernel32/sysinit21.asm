; ****************************************************************************
; SYSINIT.BIN (MSDOS 3.3 Kernel) - RETRO DOS v3.2 by ERDOGAN TAN
; ----------------------------------------------------------------------------
...
; SYSINIT21.ASM - Retro DOS v3.2, 09/07/2019 
; ------------------------------------------
...
; 10/06/2018 - Retro DOS v3.0 (MSDOS 3.3, BUFFER.INC, 1987)
; 24/07/2019 - Retro DOS v3.2 (MSDOS 6.0, BUFFER.INC, 1991)

struc BUFFINFO
; MSDOS 3.3
.buf_link:	resd 1		; Pointer to next buffer in list  ; Retro DOS v3.2
; MSDOS 6.0
...
toomanydrivesflag:
	db	0  ; >24 fixed disk partitions flag 
...
;		   = 098Fh (for floppy disks, without harddisk)
	...
	; 17/07/2019 - Retro DOS v3.2 
	; (MSDOS 6.0 SYSINIT feature, Retro DOS v4.0)
...
p_dosinit_msg:
	...
	mov	dl,[DEFAULT_DRIVE] ; 17/07/2019
...
NEXTDPB:
	...
	INC	BL
	MOV	AX,(IOCTL*256)+8  ; mov	ax,(IOCTL<<8)|8	; 4408h
	INT	21H
...
; Get parameters of drive
	...
	;mov	ax,(IOCTL<<8)|GENERIC_IOCTL ; 440Dh
...
NOSETBUF:
	;cmp	word [es:bp+19h],0FFFFh  ; MSDOS 6.0
	CMP	WORD [ES:BP+DPB.NEXT_DPB],-1
	jz	short Chk_Memsize_for_Buffers
	;les	bp,[es:bp+19h] ; MSDOS 6.0
...
Chk_Memsize_for_Buffers:
	; 26/07/2019
	cmp	word [MEMORY_SIZE],2000h ; 8192  ; 128 kb
	jbe	short BUFSET

	mov	byte [BUFFERS],15

	cmp	word [MEMORY_SIZE],8000h ; 32768 ; 512 kb	
	ja	short BUFSET ; 06/07/2019

	mov	byte [BUFFERS],10

	cmp	word [MEMORY_SIZE],4000h ; 16384 ; 256 kb
	ja	short BUFSET ; 06/07/2019
...
; retrodos33.asm (offset addresses in MSDOS.SYS or RETRODOS.SYS)
; -----------------------------------------------------------
; Note: These offset addresses must be chanqed when the code 
; 	in retrodos33.asm (MSDOS.SYS) file will be changed.

INT19SEM equ 06D5h ; 21/07/2019 - retrodos33.txt
I19_LST	 equ 06D6h ; 21/07/2019 - retrodos33.txt
INT19OLD02 equ I19_LST+1 ; 06D7h
...
INT19OLD77 equ I19_LST+66 ; 0718h

; 28/07/2019
RE_INIT	equ 1A6Bh ; 1AA6h for MSDOS 3.3 IBMBIO.COM
...
BREAKOK:
        ...
        MOV     SI,DX
        ADD     SI,SYSDEV.ATT	; 4	; DS:SI POINTS TO ATTRIBUTES
...
TRYK:
	CMP	AH,'K'
	;JNZ	short TRYW
	jnz	short TRYS ; 03/08/2019 - Retro DOS v3.2
