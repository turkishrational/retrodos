; ****************************************************************************
; RETRODOS.SYS (MSDOS 3.3 Kernel) - RETRO DOS v3.3 (for Windows 3.1)
; -Modified Retro DOS v3 for Windows 3.1 compatibility-		by ERDOGAN TAN
; ----------------------------------------------------------------------------
; Last Update: 28/08/2023 (BugFix) - (Previous: 15/09/2022)
; ----------------------------------------------------------------------------
; Beginning: 24/02/2018 (Retro DOS 2.0), 25/05/2018 (Retro DOS 3.0)
; ----------------------------------------------------------------------------
; Assembler: NASM version 2.11 (2.15)
; ----------------------------------------------------------------------------
;	    ((nasm retrodos33.s -l retrodos33.lst -o MSDOS.SYS -Z error.txt)) 	
; ----------------------------------------------------------------------------
...
;=============================================================================
; 25/08/2022 - 2022 modifications (by comparing with PC-DOS 3.30 IBMBIO.COM)
;=============================================================================
; (Modifications are done for running Windows 3.1 on Retro DOS v3.0 to v3.3)
;-----------------------------------------------------------------------------  
...
;align 256 ; 13/06/2018
	; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB) compatibility (db 169 dup(0)) 
	;times	61 db 0
...
CON$EXIT:
		...
		; Temporary !
		; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB) address compatibility!
		;db	0
...
RD1:				     		; set command to 'see if
		...
		; 25/08/2022
		jnz	short gotchr
		;nop	; IBMBIO.COM (IBMBIO33.IDB) address compatibility!
nochr:
		cmp	byte [fHaveK09], 0
		;jz	short CONBUSJ
		jz	short CONBUS
...
FlLoop:
		...
		; 25/08/2022
		;mov	ah, 0 ; IBMBIO.COM (IBMBIO33.IDB)
...
BDSH:
	...
	; Temporary !
	; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB)
	db	50h
...
popr:
		...
		; Temporary !
		; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB) compatibiliy !
		;jmp	HAS1
...
ENDK09:
	...
	; Temporary !
	; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB) address compatibility!
	;times	20 db 0 ; 'DRVFAT:' address must be 29FAh
...
; 03/09/2022
;BPBTable:
;		; 25/08/2022 - IBMBIO.COM (IBMBIO33.IDB) compatibility!
...
; 15/09/2022
BPB_Table equ SYSINIT_START + 1D38h  ; (BPBTable offset in sysinit2.s)
...
MSDOS_BIN_OFFSET:  ; this offset must be paragraph aligned
		; 15/09/2022 - Windows 3.1 compatible Retro DOS v3 kernel
		;
		; -----	MSDOS (PC-DOS) Version 3.30
		; ----- 'IBMDOS.COM' kernel content except 'IBMBIO.COM'
		;
		; Notes: Windows 3.1 does not run if addresses and -most of-
		;	content -code- aren't same with original MSDOS kernel
		;	('temporary' and 'magic code' modification notes
		;	-in 'msdos33.asm'- are for that!).
		;	((NASM puts different opcodes -than MASM- for some
		;	asm instructions. This is a reason for some problems)) 
		;	Also, original ms 'command.com' (v3.30) is needed for
		;	msdos prompt within windows (3.1). (Retro DOS v3
		;	'command.com' is not proper for dos prompt in windows)
		;	((MSDOS v5.0 'HIMEM.SYS' & 'SMARTDRV.SYS' can be used
		;	instead of Windows 3.1 'HIMEM.SYS' & 'SMARTDRV.EXE'))
		;
		;	-!- Even if -previous- Retro DOS v3 Kernel passes
		;	the AARD checkout ('MSDETECT.EXE' utility checks the 
		;	AARD compatibility), Windows 3.1 does not run due to
		;	address and opcode differences in the kernel.
		;	But, this is not as explained in Andrew Schulman's
		;	'Undocumented DOS' book, there are other barriers
		;	-for DOS v3.3 kernel- in addition to the AARD code
		;	in the 'WIN.COM'!?. -!- 
		;
		; 14/09/2022
		; 09/09/2022
		; 06/09/2022 (2022 modification for windows 3.1 running)
		; 22/07/2019 ('msdos3.s' last modification)
		; 20/05/2019 (BugFixed MSDOS3.BIN)
		; 10/09/2018 ('msdos3.s') 
		incbin	'MSDOS33.BIN' ; 05/07/2019 ; 15/09/2022
